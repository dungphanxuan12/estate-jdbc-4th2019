...
public class BuildingRepository extends AbstractJDBC<BuildingEntity> implements IBuildingRepository {
	...
	public Long insert(BuildingEntity buildingEntity) {
		StringBuilder sql = new StringBuilder("INSERT INTO building (");
		sql.append("name,ward,street,numberofbasement,buildingarea,district,structure,costrent,costdescription,servicecost,");
		sql.append("carcost,motobikecost,overtimecost,electricitycost,deposit,payment,timerent,timedecorator,managername,managerphone,type");
		sql.append(",createdby,modifiedby,createddate,modifieddate)");
		sql.append("VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		return this.insert(sql.toString(),buildingEntity.getName(),buildingEntity.getWard(), buildingEntity.getStreet(),
				buildingEntity.getNumberOfBasement(), buildingEntity.getBuildingArea(),
				buildingEntity.getDistrict(), buildingEntity.getStructure(),
				...
				buildingEntity.getManagerPhone(), buildingEntity.getType(),
				buildingEntity.getCreatedBy(), buildingEntity.getModifiedBy(),
